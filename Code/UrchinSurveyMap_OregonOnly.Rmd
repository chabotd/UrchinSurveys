...
#read.csv("Data/IntertidalUrchinSites.csv")

#you will have to do this by hand in command line for some reason?
...
Site <-SiteList$Site
Reg <-SiteList$Region
Reg<-factor(Reg, levels=c('Cape Foulweather','Cape Perpetua','Cape Arago','Cape Blanco', 'Southern Oregon'))
...
map <-ggplot(data = SiteList, 
             aes(x=long, 
                 y = lat, color = Region)) +   
 geom_text_repel(
  aes(x = long, y = lat, label = SiteCode, color = Region),
  direction = "y", 
  nudge_x = -125, 
...
plot(map)
#again have to save ggsave in command line
