...
long_subset <- SiteList_subset$Longitude
lat <- SiteList$Latitude
lat_subset <- SiteList_subset$Latitude
Site <-SiteList$Site
Reg <-SiteList$Region
Reg_subset <- SiteList_subset$Region
...
Reg_subset <- factor(Reg_subset, levels=c('Cape Foulweather','Cape Perpetua','Cape Arago','Cape Blanco', 'Southern Oregon'))
color_subset <- c("#90be6d","#f8961e", "#f94144", "#555599", "#66BBBB")

SiteCode <-SiteList$SiteCode
SiteCode_subset <- SiteList_subset$SiteCode
```

Andrew Carsten Moreland makes a subset minus Nor Cal survey sites
(For Ella's HC thesis w 10 survey plots in OR)
```{r}
SiteList_subset <- SiteList[-c(11,12),]
print(SiteList_subset)
```
...
Drella version
```{r}
map <-ggplot(data = SiteList_subset, 
             aes(x=long_subset, 
                 y = lat_subset, color = Reg_subset)) +   
  geom_text_repel(aes( label = SiteList_subset$SiteCode),
                  direction = "y", 
                   nudge_x = -125, 
                   min.segment.length = 0, 
                   seed = 42, 
  ...
  aes(x = long, y = lat, group = group),  # use correct columns
  color = "black",
  fill = "white",
  inherit.aes = FALSE) +
  geom_point(size = 2, shape = 16) +
